...
- a per-user private dotfiles repo (cloned onto the machine with a fresh per-machine SSH deploy key)
...
|---|---|---|
| **Fresh machine** | No dotfiles checkout found anywhere | Resolves identity → prompts to clone your `dotfiles-<username>` (or skip). Walks through deploy key registration. Runs `install.sh`. Offers to chain into a new project (prompts for name + location, default base `~/dev_code/`). |
...
| `BOOTSTRAP_WORK_DIR` | `bootstrap` | Folder name under `$HOME` for the dotfiles checkout. No legacy-name fallbacks — if you've used a different name before, set this env var explicitly. |
...
If `gh` (the GitHub CLI) is installed, `bootstrap.sh` uses it for registering deploy keys (`gh repo deploy-key add`).
...
- The dotfiles deploy key is scoped to your `dotfiles-<username>` repo. Default is read-only (consumer-machine model — clone but don't push). If you want your primary editing machine to push dotfiles updates from this machine, register the key with `--allow-write` manually via `gh repo deploy-key add ~/.ssh/dotfiles-amin-lakhani_ed25519.pub -R <user>/<repo> --allow-write` after bootstrap. The current key from disk is exposed via the SSH alias either way.
...
If you want to share this setup, the path is: branch this repo (`project-bootstrap-template`) and your own dotfiles repo, scrub anything personal, publish the branches as templates. The cleanest version is what's on `main`; the personal customization lives in your fork. Students would then use the quickstart snippet pointed at *your* `project-bootstrap-template` fork and clone-or-skip *their* dotfiles repo.
...
- `bootstrap.sh` — the single entry point (~860 lines, structured into clearly-delimited sections: logging, browser/clipboard helpers, identity, gh auth + cleanup trap, dotfiles flow, per-project setup, state detection, main)
- `README.md` — this file
