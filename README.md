...
- The dotfiles deploy key is scoped to your `dotfiles-<username>` repo. Default is read-only (consumer-machine model — clone but don't push). If you want your primary editing machine to push dotfiles updates from this machine, register the key with `--allow-write` manually via `gh repo deploy-key add ~/.ssh/dotfiles_ed25519.pub -R <user>/<repo> --allow-write` after bootstrap. The current key from disk is exposed via the SSH alias either way.
